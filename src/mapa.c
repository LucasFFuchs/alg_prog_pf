...
// -> o arquivo mapa.c gerencia completamente o sistema de mapas do jogo
// aqui nasce o cenario onde tudo acontece: terreno, caminho do rio
// posicao inicial do jogador e onde surgem os inimigos
// mapa.c le o arquivo de texto da fase, monta a matriz do mapa e desenha cada tile na tela
// ele tambem posiciona o jogador no ponto inicial ('A') e ativa inimigos definidos no mapa
        ...
        for(int coluna = 0; coluna < COLUNA; coluna++){
            ...
            switch(t.tipo){
                ...
                case 'N':   if(!flag)
                            ...
                            break;
                case 'A':   if(!flag)  // Tile de spawn do jogador (ja tratado em InitMapMatrix)
                                 // Nada a fazer graficamente
                            break;

            }
        }
    }
    flag++; // Impede que inimigos sejam criados varias vezes
}
